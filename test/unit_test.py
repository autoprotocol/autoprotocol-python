...
class TestUnitMath:
    ...
    def test_conversion(self):
        ...
        assert (Unit(1000, 'microliter').to('milliliter') ==
                Unit(1, 'milliliter'))
    ...
    def test_autoprotocol_format(self):
        ...
        assert (str(Unit(2, 'millimeter') * Unit(2, 'millimeter')) ==
                "4:millimeter^2")

    def test_string_rounding(self):
        # Ensure that string representation is rounded
        assert (str(Unit(0.1, 'microliter') + Unit(0.2, 'microliter')) ==
                "0.3:microliter")
        # Currently we round to 12 decimal places by default
        assert(str(Unit(0.1234567890123, 'microliter')) ==
               "0.123456789012:microliter")
        assert (str(Unit(0.1234567890126, 'microliter')) ==
                "0.123456789013:microliter")
